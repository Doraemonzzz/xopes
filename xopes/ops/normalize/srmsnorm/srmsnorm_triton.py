...
# fmt: off
@triton.jit
def srms_norm_fw(X, Y, V, stride, N, eps, scale: tl.constexpr, BLOCK_SIZE_N: tl.constexpr):
    # fmt: on
    row = tl.program_id(0)
    ...
    if scale: # equivalent to l2 norm
        c = 1
    else:
        c = N

    x_var = tl.sum(x_zm * x_zm, axis=0) / c
...
def srms_norm_bwd_dx_fused(
    ...
    mean1 = tl.sum(xhat * wdy, axis=0) / c
...
def srmsnorm_triton(x, eps=1e-8, scale=False):
    return _SrmsNorm.apply(x, eps, scale)
