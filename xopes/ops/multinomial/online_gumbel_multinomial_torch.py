...
@torch.no_grad()
def online_gumbel_multinomial_torch(x, W, num_samples, lse=None, block_size=1024):
    d, V = W.shape
    b = x.shape[0]
    ...
    logits = torch.full((b, block_size), -float("inf"), dtype=x.dtype, device=x.device)
    max_value = torch.full((b, 1), -float("inf"), dtype=torch.float32, device=x.device)
    ...
    for i in range(m):
        ...
        max_value = torch.max(
            torch.stack([max_value, max_value_curr], dim=-1), dim=-1
        ).values
        torch.exp(logits_curr - lse_curr)
...
if __name__ == "__main__":
    ...
    x = torch.randn(b, d)
    W = torch.randn(d, V)
    sample = online_gumbel_multinomial_torch(x, W, num_samples)
