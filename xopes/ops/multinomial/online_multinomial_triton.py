...
def _online_multinomial_triton(
    ...
    # BLOCK_D, BLOCK_V
    w_block_ptr = (
        W + tl.arange(0, BLOCK_D)[:, None] * d + tl.arange(0, BLOCK_V)[None, :]
    )
    # BLOCK_B, k
    sample_block_ptr = (
        Sample
    ...
    # for random
    # BLOCK_B, 1, k
    rand_block_ptr1 = tl.zeros([BLOCK_B, 1, k], dtype=tl.float32)
    # BLOCK_B, k
    rand_block_ptr2 = tl.zeros([BLOCK_B, k], dtype=tl.float32)
    ...
    sample = tl.zeros([BLOCK_B, BLOCK_K], dtype=tl.int64)
    ...
    for i in range(tl.cdiv(v, BLOCK_V)):
        ...
        upper = prob_cum_curr[:, :, None] >= p
        # (BLOCK_B, k)
        sample_curr = tl.argmin(upper, axis=1)
        # tl.static_print("bbb", sample_curr)
    ...
    tl.store(sample_block_ptr, sample, mask=k_mask[None, :])
...
def online_multinomial_triton(x, W, num_samples, lse=None, max_value=None):
    ...
    BLOCK_K = next_power_of_two(num_samples)
    seed = 0
