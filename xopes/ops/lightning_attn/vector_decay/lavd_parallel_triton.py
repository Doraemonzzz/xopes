...
def lavd_parallel_sub_intra(
    ...
    # if use_ldv:
    #     tmp = torch.cumsum(ldv, dim=1)
    #     print("diff", torch.norm(tmp - ldv_cumsum))
    #     o = o * torch.exp(ldv_cumsum)
...
def lavd_parallel_fwd(
    ...
    return o, states, ldk_cumsum, ldv_cumsum
...
def lavd_parallel_bwd(
    ...
    _, dldv_o = lavd_parallel_intra_inter(
        q=q,
        k=k,
        v=v,
        states=states,
    ...
    dq, dldk_q = lavd_parallel_intra_inter(
        q=do,  # b n h e
        k=v,  # b n h e
        v=k,  # b n h d
    ...
    dv, dldv_v = lavd_parallel_intra_inter(
        q=k,  # b n h d
        k=q,  # b n h d
        v=do,  # b n h e
        states=dstates,  # b h (m + 1) d e
...
class LavdParallelFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        # Forward computation
        output, states, ldk_cumsum, ldv_cumsum = lavd_parallel_fwd(
            q=q,
            k=k,
            v=v,
            ldk=ldk,
