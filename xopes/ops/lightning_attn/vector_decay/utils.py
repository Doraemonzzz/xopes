...
def _lavd_parallel_intra_inter_no_loop(
    ...
    if REVERSE:
        ...
        offset_ldk_sum = offset_ld2 * H * D
        # offset_ldk_sum = offset_ld1 * H * D
    ...
    if USE_DECAY_K:
        ...
        if REVERSE:
            offset_ldq_start = offset_ld2
            mask_ldq_start = (offset_ldq_start < BLOCK_N) & (
                (offset_block_n + offset_ldq_start) < N
            )
        else:
            offset_ldq_start = offset_ld1
            mask_ldq_start = (offset_ldq_start >= 0) & (
                (offset_block_n + offset_ldq_start) < N
            )
    ...
    for i in range(NUM_BLOCK_D):
        ...
        if USE_DECAY_K:
            ...
            # sub inter decay
            log_decay_q_start = tl.load(
                ldq_start_block_ptr,
                mask=mask_ldq_start & mask_d[None, :],
                other=0.0,
            ).to(tl.float32)
            ...
            q1_decay_sub = tl.exp(log_decay_q1_sub - log_decay_q_start)
            q1_ = (q1 * q1_decay_sub).to(q1.dtype)
            score1 = tl.dot(q1_, k_trans).to(q1.dtype)
            ...
            q2_decay_sub = tl.exp(log_decay_q2_sub - log_decay_q_start)
            q2_ = (q2 * q2_decay_sub).to(q2.dtype)
            score2 = tl.dot(q2_, k_trans).to(q2.dtype)
