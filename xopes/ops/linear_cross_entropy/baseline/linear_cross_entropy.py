# credit to: https://github.com/JonasGeiping/linear_cross_entropy_loss/blob/main/linear_cross_entropy/ops.py
import math
...
def linear_xent_fwd_prep_bwd_kernel_matmul_t(
    ...
    mask = (
        y[:, None] == tl.where(V_range != ignore_index, V_range, -1)[None, :]
    )  # Nc x Vc
...
class LinearXentImplementation(torch.autograd.Function):
    ...
    def forward(
        ...
        with torch.cuda.device(
            x_in.device.index
        ):  # actually required for devices other than 0
