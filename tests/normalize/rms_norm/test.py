...
def test(shape, use_residual, return_residual, c, eps, dtype):
    ...
    if isinstance(o_triton, tuple):
        o_rms_norm_triton, o_update_residual_triton = o_triton
    else:
        o_rms_norm_triton = o_triton
