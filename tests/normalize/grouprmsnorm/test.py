...
from xopes.ops.normalize import grouprmsnorm_torch, grouprmsnorm_triton
...
def test(shape, use_residual, return_residual, eps, dtype, num_groups):
    ...
    # forward
    o_rmsnorm_torch, o_update_residual_torch = grouprmsnorm_torch(
        x,
        weight=weight,
        bias=bias,
        dim=d,
    ...
    o_rmsnorm_triton, o_update_residual_triton = grouprmsnorm_triton(
        x,
        weight=weight,
        bias=bias,
        dim=d,
    ...
    db_rmsnorm_torch, bias.grad = bias.grad.clone(), None
    ...
    db_rmsnorm_triton, bias.grad = bias.grad.clone(), None
    ...
    print(
        "db diff max: ",
        torch.abs(db_rmsnorm_torch - db_rmsnorm_triton).max().item(),
    )
    print(
        "db diff norm: ",
        torch.norm(db_rmsnorm_torch - db_rmsnorm_triton).item(),
    )
    assert torch.allclose(db_rmsnorm_torch, db_rmsnorm_triton, atol=atol, rtol=rtol)
