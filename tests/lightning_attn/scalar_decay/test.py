...
def test(shape, use_initial_state, use_log_decay, use_varlen, act, norm, dtype):
    ...
    o_parallel_triton.sum() + s_parallel_triton.sum()
    ...
    print("aaa", torch.min(s_torch), torch.max(s_torch))
    print("bbb", torch.min(s_parallel_triton), torch.max(s_parallel_triton))
    # assert False
    # m = 64
